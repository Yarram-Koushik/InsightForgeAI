...
def build_schema_context(workspace, table_name: str, max_sample_rows: int = 5) -> str:
    ...
    try:
        ...
        sys.modules["schema_mod"] = schema_mod
...
def generate_sql(
    ...
    user_parts = ["SCHEMA:", schema_ctx, "", f"QUESTION: {question.strip()}"]
    try:
        import importlib.util as _ilu
        _model, _sl = _get_semantic_model(workspace, table_name)
    ...
    if previous_error and previous_sql:
        user_parts.extend([
            "", "PREVIOUS SQL (failed):", previous_sql, "",
    ...
    user_prompt = "\n".join(user_parts)
    response = client.chat(system_prompt=SYSTEM_PROMPT, user_prompt=user_prompt, temperature=0.05, max_tokens=800)
...
def ask(workspace, table_name: str, question: str, max_retries: int = 2) -> NL2SQLResult:
        ...
        if _mc_path.exists():
            ...
            if _compiled.success and _compiled.sql:
                ...
                if exec_error is None:
                    ...
                    result.result_df = df
                    result.attempts = 0
                    result.explanation = f"Deterministic metric compile (Phase 3.2). {_compiled.explanation}"
    ...
    for attempt in range(1, max_retries + 2):
        ...
        sql, llm_resp = generate_sql(workspace=workspace, table_name=table_name, question=question, previous_error=last_error, previous_sql=current_sql)
        ...
        if exec_error is None:
            ...
            if df is not None and len(df) == 0:
                result.warnings.append("Query executed successfully but returned 0 rows.")
            result.explanation = f"Executed successfully via {llm_resp.provider} ({llm_resp.model}). {len(df):,} row(s) returned."
            return result
        last_error = exec_error
        if attempt > max_retries:
            result.error = f"SQL still failed after {max_retries} correction attempt(s).\nLast error: {exec_error}"
